...
def upgrade() -> None:
    ...
    op.create_unique_constraint(
        "attendance_event_instance_id_user_id_is_planned_key",
        "attendance",
        ["event_instance_id", "user_id", "is_planned"],
    )
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column(
        "events", sa.Column("backblast_ts", sa.DOUBLE_PRECISION(precision=53), autoincrement=False, nullable=True)
    )
    op.add_column(
        "events",
        sa.Column("is_series", sa.BOOLEAN(), autoincrement=False, nullable=False, server_default=sa.text("false")),
    )
    op.add_column(
        "events", sa.Column("preblast_ts", sa.DOUBLE_PRECISION(precision=53), autoincrement=False, nullable=True)
    )
    op.add_column("events", sa.Column("fng_count", sa.INTEGER(), autoincrement=False, nullable=True))
    op.add_column(
        "events", sa.Column("preblast_rich", postgresql.JSON(astext_type=sa.Text()), autoincrement=False, nullable=True)
    )
    op.add_column("events", sa.Column("backblast", sa.VARCHAR(), autoincrement=False, nullable=True))
    op.add_column(
        "events",
        sa.Column("backblast_rich", postgresql.JSON(astext_type=sa.Text()), autoincrement=False, nullable=True),
    )
    ...
    op.create_unique_constraint(
        "attendance_event_id_user_id_is_planned_key", "attendance", ["event_id", "user_id", "is_planned"]
    )
